...
Serial.begin(9600);
  ...
  display.display();
  delay(50); // Pause for 0,8 seconds
  display.clearDisplay();
  // Clear the buffer
  delay(50); // Pause for 0.5 seconds
  display.drawBitmap(32, 0, logo_bmp, 64, 64, 1); //show the Moe Bootscreen
  display.display();
  delay(400); // Pause for 4 seconds
  display.clearDisplay();
...
void loop() {
 ...
 if (done == 0){  
  ...
  display.display();
  delay(15);
    myservo1.write(180);                // tell servo1 to go to Zero (brake if switch is enabled
    myservo2.write(180);           
    delay(5000);                        // waits 5s for the ESC to enter Setup
display.clearDisplay();
  display.setTextColor(SSD1306_WHITE);        // Draw white text
  display.setTextSize(1);
  display.setCursor(3,5);
  display.println("Teaching zero");
  display.display();
  delay(15);
  ...
  delay(15);
    ...
    myservo2.write(180);           
    delay(5000);                    
  done = 1;                            // Finished the script, exiting the config loop
  }
  ...
  display.println("Unplug now");
  display.display();
  delay(15);
  
  myservo1.write(0);                 // sets the servo1 position according to the scaled value
  myservo2.write(0);                 // sets the servo2 position according to the scaled value
